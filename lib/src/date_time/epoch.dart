...
//
import 'package:constants/src/date_time/constants.dart';
...
final int kMinYearInMicroseconds =
    dateToEpochDay(minYear, 1, 1) * kMicrosecondsPerDay;
final int kMaxYearInMicroseconds =
    dateToEpochDay(maxYear, 1, 1) * kMicrosecondsPerDay;

/// The minimum Unix Epoch day for this epoch.
final int kMinEpochDay = _dateToEpochDay(minYear, 1, 1);

/// The maximum Unix Epoch day for this epoch.
final int kMaxEpochDay = _dateToEpochDay(maxYear, 12, 31);
...
int dateToEpochDay(int y, int m, int d) =>
    (_isValidDate(y, m, d)) ? _dateToEpochDay(y, m, d) : _badDate(y, m, d);
...
int _dateToEpochMicroseconds(int y, int m, int d) {
  ...
  return (isValidEpochDay(day)) ? day * kMicrosecondsPerDay : _badDate(y, m, d);
}
...
bool _isValidYear(int y) => _inRange(y, minYear, maxYear);

//TODO: remove if only used in tests
/// Returns [day] if valid; otherwise, throws and Exception.
int checkEpochDay(int day) => _isValidEpochDay(day) ? day : _badEpochDay(day);
...
class Epoch {
  final int minYear;
  final int maxYear;
  final int minYearInMicroseconds;
  final int maxYearInMicroseconds;

  Epoch(this.minYear, this.maxYear)
      : minYearInMicroseconds = minYear * kMicrosecondsPerYear,
        maxYearInMicroseconds = maxYear * kMicrosecondsPerYear;

  bool isValidYear(int year) => year >= minYear && year <= maxYear;

  @override
  String toString() => 'Epoch(minyear = $minYear, maxYear = $maxYear';
}
...
Object _epochDayToEpochDate(int epochDay) {
  if (isNotValidEpochDay(epochDay)) return _badEpochDay(epochDay);
...
int hashDateMicroseconds(int us, [int Function(int n) onError]) {
...
int hash(int epochDay, [int Function(int n) onError]) {
  final v = hash(epochDay);
...
String epochDayToDateString(int epochDay, {bool asDicom = true}) {
  ...
  if (date == null) _badEpochDay(epochDay);
...
int _badDate(int y, int m, int d) =>
    throw Exception('Invalid date: y= $y m= $m, d= $d');

int _badEpochDay(int epochDay) => throw Exception('Invalid epochDay $epochDay');
